...
def main() -> None:
    # JSON to DF
    df = pd.read_json(
        "/Users/karolinaryzka/Documents/neo4jQueries/Data/outDegreeFrequency.json",
        encoding="utf-8-sig",
    )
    ...
    def plotRange(minFreq, maxFreq, title, ylabel_formatter, y_major_locator):
        ...
        graph.set_title(
            f"Reference Frequency in Papers ({int(minFreq)}-{int(maxFreq)})"
        )
        ...
        plt.tight_layout()
        plt.savefig(
            f"/Users/karolinaryzka/Documents/neo4jQueries/Visualizations/outDegreeDistributions/outDegreeDistr_{int(minFreq)}_{int(maxFreq)}.png"
        )
