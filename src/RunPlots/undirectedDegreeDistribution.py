...
def main() -> None:
    #JSON to DF
    df = pd.read_json('/Users/karolinaryzka/Documents/neo4jQueries/Data/allDegreeFrequencies.json', encoding='utf-8-sig')

    
    def plotRange(minFreq, maxFreq, title, ylabel_formatter, y_major_locator):
        rangeDf = df[(df['frequency'] >= minFreq) & (df['frequency'] <= maxFreq) & (df['frequency'] >= 10)]
        #rangeDf = df[(df['frequency'] >= minFreq) & (df['frequency'] <= maxFreq)]
        plt.figure(figsize=(12, 6))
        sns.set_theme()
        graph = sns.barplot(data=rangeDf, x='degree', y = 'frequency' )
        graph.set_ylabel('Frequency')
        graph.set_xlabel('Number of Interactions with Paper')
        graph.set_title(f'Frequency of Interactions in Papers (Citations and References) ({int(minFreq)}-{int(maxFreq)})')
        ...
        plt.tight_layout()
        plt.savefig(f"/Users/karolinaryzka/Documents/neo4jQueries/Visualizations/undirectedDegreeDistributions/undirectedDegreeDistr_{int(minFreq)}_{int(maxFreq)}.png")
